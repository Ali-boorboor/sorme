...
import { ProductBoxPropsType } from "@/types/MoleculesType";
import { useRouter } from "next/router";
import { memo } from "react";

const ProductBox = memo((props: ProductBoxPropsType) => {
  ...
  return (
    ...
    >
      <div className="basis-1/2 flex flex-col gap-2 items-start justify-center">
        <img src={props.cover} alt="product-img" className="h-40 w-32 object-cover" />
        <p className="w-48 font-normal text-black text-base">{props.text}</p>
      </div>
      <div className="basis-1/2 flex gap-2 items-end">
        <p className="font-bold text-lg text-primary-color">{props.price}$</p>
        <div className="flex flex-col items-center gap-4 h-full justify-between">
          <p className="font-normal text-black text-base">{props.usersCount} person</p>
